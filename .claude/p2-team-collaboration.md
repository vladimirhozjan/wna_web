...
- **Tags are always private** — even on shared projects and shared project actions, your tags are only visible to you. Other members cannot see your tags. Each member can set their own tags on shared projects and on the actions inside them (private, not visible to others). This applies to **both** the project entity itself and every action in the project: write members and read-only members alike can curate their own private tag set, even though they cannot edit the project's owner-only fields (title, outcome, description).
...
| # | Story | Priority |
|---|-------|----------|
| T-43 | As the system, I enforce team tier limits (1 GB storage per user, 50 MB attachments). Attachments uploaded by a member to a shared project are attributed to and counted against the **project owner's** storage (see §11 "Shared-project attachments attribute to the owner"). | Must |
...
- **Tags are always private**: Even on shared projects and shared project actions, tags are per-user and invisible to other members.
    - **Action tags** (on actions inside a shared project): stored per-user per-action in `action_user_tags(action_id, user_id, tags)`. NOT on `action.tags`. When a user unassigns an action, their tags are discarded. Each member can set their own tags on any shared-project action (private, no inheritance to other members).
    - **Project tags** (on the project entity itself): stored per-user per-project in `project_user_tags(project_id, user_id, tags)`. NOT on `project.tags` for shared projects. Every project member (owner, write, read-only) can curate their own tag set on the project even though only the owner can edit the project's title/outcome/description. The PUT `/v1/project/{id}` endpoint accepts `tags` from non-owner members provided the body contains **only** the `tags` field; sending `tags` together with title/outcome/description returns 403 (atomic semantics — the frontend splits the call). Reads through `GET /v1/project/{id}` and `GET /v1/projects` return the caller's own tags on shared projects (empty array if the caller has no row yet).
    - **Conversion**: when a personal project becomes shared, the owner's existing `project.tags` is migrated into `project_user_tags(project, owner)` and `project.tags` is cleared. When a shared project converts back to personal, the owner's `project_user_tags` row is migrated back into `project.tags`; other members' per-user rows are dropped along with their membership. Same shape applies to actions during conversion.
    - **Member removal**: when a member is removed (or self-leaves), their `project_user_tags` and `action_user_tags` rows for that project are dropped — they're private and only meaningful while membership exists.
...
- **Shared-project content attributes to the project owner** (not the uploader). Any artefact a member creates inside a shared project — backlog actions, attachments, and the associated reference-file/storage rows — is stored under the **project owner's** `user_id`. Consequences:
    - Attachment uploads by any member land in the **owner's** storage namespace, count against the **owner's** storage quota, and the `attachment.user_id` row stores the owner's id. Delete-quota refunds also credit the owner.
    - Backlog actions a write member creates have `item_meta.user_id = owner` so they remain in the project after the member leaves.
    - Tier-driven per-file size limit is checked against the **uploader's** tier (they're the actor); storage-quota total is checked against the **owner's** tier limit. Asymmetry is intentional: the owner pays for the storage that lives in their space.
    - Comments are the exception: `comment.user_id` stays as the author (so the UI can render "who said what"). Comments persist on the action regardless of author membership; if the author is deleted, the comment is shown attributed to "Deleted User." See above.
    - This rule means a write member can leave (or be removed from) a shared project without taking project content with them. Their private per-user data — `action_user_tags`, their own claim if any — is the only thing that becomes orphaned/unassigned, per existing rules.
