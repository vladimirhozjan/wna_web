    ...
    <div class="today-page">
      ...
      <div class="today-content">
        ...
        >
          ...
          <template #empty>
            <TodayIcon class="empty-state__icon" />
            <h2 class="empty-state__title">No actions for today</h2>
            <p class="empty-state__text">
              Move actions here to focus on what matters most today.
            </p>
          </template>
        </ItemList>
      </div>
...
function onItemClick(item, index) {
  router.push({
    name: 'action-detail',
    params: { id: item.id },
    query: { position: index, total: totalItems.value, from: 'today' }
  })
}
...
async function onMoveToSection(id, destination) {
  ...
  try {
    if (destination === 'NEXT') {
      await apiClient.undeferAction(id) // Remove from Today
